...
IF( (   COUNT([Detail_Bonregel artikel_Order].[Omschrijving])+
        ...
        COUNT([Detail_Bonregel uitbesteed werk_Order].[Omschrijving]))==0,"Bon is leeg",


IF( COUNT([Detail_Materiaalstatus_Order].[Materiaal ID])==
    COUNTCONDITION([Detail_Materiaalstatus_Order].[Materiaal ID],[Detail_Materiaalstatus_Order].[Onderhanden werk]==[$MaterialState.Waar]), "Alles OHW",
IF( COUNT([Detail_Materiaalstatus_Order].[Materiaal ID])==
